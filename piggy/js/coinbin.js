$(document).ready(function() {
  ...
  console.log(pUrl);
	...
	function walletBalance(){
        ...
        $.ajax ({
			type: "GET",
			url: "https://cryptodepot.org:8083/chainz/balance/piggy/"+$("#walletAddress").html()+"",
			dataType: "json",
			error: function(data) {
	...
	function listUnspentInsightAPI_PiggyCoin(redeem){
		$.ajax ({
			type: "GET",
			url: "https://cryptodepot.org:8083/chainz/listunspent/piggy/"+ redeem.addr,
			dataType: "json",
			error: function(data) {
				$("#redeemFromStatus").removeClass('hidden').html('<span class="glyphicon glyphicon-exclamation-sign"></span> Unexpected error, unable to retrieve unspent outputs!');
			},
			success: function(data) {
				...
				//if(data && data.length){
					$("#redeemFromAddress").removeClass('hidden').html(
						'<span class="glyphicon glyphicon-info-sign"></span> Retrieved unspent inputs from address <a href="https://chainz.cryptoid.info/piggy/address.dws?'+redeem.addr+'" target="_blank">'+redeem.addr+'</a>');
	...
	function rawSubmitInsightAPI_PiggyCoin(thisbtn){
		...
		$.ajax ({
			type: "POST",
			url: "https://chainz.cryptoid.info/piggy/api.dws?q=pushtx",
			data: {"rawtx":$("#rawTransaction").val()},
			dataType: "json",
			error: function(data) {
