	...
	$("#walletConfirmSend").click(function(){
		...
		tx.addUnspent($("#walletAddress").html(), function(data){
			...
			if(dvalue>=total){
				...
				$("#walletSendConfirmStatus").removeClass("hidden").addClass('alert-danger').html("You have a confirmed balance of "+dvalue+" BTG unable to send "+total+" BTG").fadeOut().fadeIn();
				thisbtn.attr('disabled',false);
			}
	...
	function walletBalance(){
		...
		coinjs.addressBalance($("#walletAddress").html(),function(data){
			if($(data).find("result").text()==1){
				...
				$("#walletBalance").html(v+" BTG").attr('rel',v).fadeOut().fadeIn();
			} else {
				$("#walletBalance").html("0.00 BTG").attr('rel',v).fadeOut().fadeIn();
			}
